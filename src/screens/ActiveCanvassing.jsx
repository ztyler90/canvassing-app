...
import { endSession, updateSessionStats } from '../lib/supabase.js'
...
export default function ActiveCanvassing() {
  ...
  const handleStop = async () => {
    ...
    await endSession(state.session.id, summary)
    ...
    <div className="h-screen flex flex-col bg-gray-100 overflow-hidden">
      ...
      <div className="px-4 py-3 flex items-center justify-between shadow-sm z-10"
        ...
        <div className="flex items-center gap-1 text-white">
          ...
          <span className="text-green-300 text-sm ml-0.5">doors</span>
        </div>
        <div className="text-white text-right">
          <span className="text-lg font-bold">${state.stats.revenue.toFixed(0)}</span>
          <span className="text-green-300 text-sm ml-0.5">booked</span>
        </div>
      </div>
